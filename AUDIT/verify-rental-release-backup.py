#!/usr/bin/env python3
"""REVIEWED EXECUTION ONLY: production backup and isolated rental-extension verification.
...
SOURCE_REVISION = "bd6d7e7c08ff40eb4953bda6b7938b0da4ca2218"
BASE_REVISION = "e9b3d7a5c201"
CANDIDATE_REVISION = "f2c8a4d6e901"
CANDIDATE_SHA256 = "a7d1415740f19872a984145a2fac46aa75c0ace8e00a901bf9660a3964455ec8"
REPO = Path("/opt/safr/safr-bali")
BACKUP_PARENT = Path("/var/backups/safr-bali-rentals")
RESULT_MARKER = "BALI_RENTALS_RESULT="
ISOLATED_NAME = re.compile(r"^bali_rentals_verify_[a-f0-9]{24}$")
RENTAL_COLUMNS = ("housing_type", "rental_mode", "quantity")
...
def assert_source(connection):
    ...
    require(connection.execute("""
        ...
        WHERE table_schema='public' AND table_name='life_services'
          AND column_name = ANY(%s)
    """, (list(RENTAL_COLUMNS),)).fetchone()[0] == 0, "Rental columns already exist; stop for review")
...
def fingerprint(connection, *, upgraded=False):
    ...
    for schema, table, owner in tables:
        if (schema, table) == ("public", "alembic_version"):
            continue
        is_rental = upgraded and (schema, table) == ("public", "life_services")
        # Exclude ONLY the three reviewed additive fields, and ONLY after upgrade.
        ...
        if is_rental:
            ...
            require(len(added) == 3, "Expected exactly three rental columns")
...
def sequence_fingerprint(connection, *, include_values=False):
...
def assert_rental_defaults(connection):
    require(connection.execute("""
        SELECT count(*) FROM public.life_services
        WHERE housing_type IS NOT NULL OR rental_mode != 'fixed' OR quantity != 1
    """).fetchone()[0] == 0, "Migration changed legacy rental defaults")
    columns = connection.execute("""
        SELECT column_name,data_type,is_nullable,column_default,character_maximum_length
        FROM information_schema.columns WHERE table_schema='public' AND table_name='life_services'
          AND column_name = ANY(%s) ORDER BY column_name
    """, (list(RENTAL_COLUMNS),)).fetchall()
    require(columns == [
        ("housing_type", "character varying", "YES", None, 16),
        ("quantity", "integer", "NO", "1", None),
        ("rental_mode", "character varying", "NO", "'fixed'::character varying", 8),
    ], "Rental column definitions differ from reviewed migration")
...
def worker(params):
    ...
    for operation, old, new in (("upgrade", BASE_REVISION, CANDIDATE_REVISION),
        ...
        with connect(params, params["isolated_db"]) as check:
            ...
            current = fingerprint(check, upgraded=operation == "upgrade")
            require(current == params["baseline"], "Original data/columns/ownership changed")
            require(sequence_fingerprint(check, include_values=True) == restored_sequences,
...
def main():
    ...
    backup = Path(tempfile.mkdtemp(prefix=f"bali-rentals-{timestamp}-", dir=BACKUP_PARENT))
    ...
    isolated = f"bali_rentals_verify_{uuid.uuid4().hex[:24]}"
    params.update(isolated_db=isolated, marker=f"BALI-LIFE rentals verification {isolated}", baseline=baseline["tables"],
                  baseline_sequences=baseline["sequences"], candidate_source=candidate_source)
