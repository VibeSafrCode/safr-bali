...
| `AUD-RISK-010` | P0 | `LOCAL_ONLY` | Visa reminder materialization reads a reminder-timezone setting that is not present in the current local Settings definition. This is a concrete runtime/test blocker risk. | `backend/app/services/visa_lifecycle.py`; `backend/app/core/config.py` | CTO | Correct configuration contract plus direct tests; do not infer PASS from source presence |
| `AUD-RISK-011` | P1 | `LOCAL_ONLY` | No scheduler/invocation for reminder materialization and no Visa Cabinet frontend/bot changes are present in the current BALI-TASK-055 diff. Full Stage 1 journey completeness is therefore unproven. | Current local diff and code-reference search | CTO + CPO | Exact scope map, scheduler/worker decision, UI/bot handoff evidence, and end-to-end fixture tests |
