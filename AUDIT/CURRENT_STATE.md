...
| 2026-09-03 calculator/FX lock hotfix | `DEPLOYED / VERIFIED` | Production code `d9f23291dffe00b40c6b5ef350664ad82b33e0f3`; request-path FX refresh removed, refresh lock is non-blocking, FX service is bounded; calculator/auth/health/catalog and parallel smoke PASS; FX `4042` LIVE | No migration, customer-data mutation or customer message was used |
...
- 2026-09-03 incident hotfix targeted regression `27 passed`; full backend run
  `171 passed`, `11 skipped`, with only the environment-dependent local database
  connectivity smoke failing. Production database health, calculator/auth routes,
  20 parallel health and 20 parallel catalog requests PASS. FX timer published
  LIVE version `4042`; advisory waiters and long idle transactions both `0`.
...
unrelated, preserved and excluded. Production checkout is `d9f2329…`; deployed
