...
for (const viewport of [{ name: "iphone-390", width: 390, height: 844 }, { name: "desktop-1440", width: 1440, height: 900 }]) {
  test(`Admin safety states ${viewport.name}`, async ({ page }) => {
    ...
    await page.route("**/api/web/admin/visa-cases/41/publication/publish", async (route) => { await new Promise((resolve) => setTimeout(resolve, 250)); await route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ ...ruVisa, publication_status: "PUBLISHED", version: 3 }) }); });
    ...
    await page.getByRole("button", { name: "Опубликовать" }).click(); await page.screenshot({ path: `${artifactRoot}/${viewport.name}/admin-publish-confirm.png`, fullPage: true });
    await page.getByRole("button", { name: "Подтвердить" }).click(); await expect(page.getByRole("button", { name: "Выполняем…" })).toBeDisabled(); await page.screenshot({ path: `${artifactRoot}/${viewport.name}/admin-publish-pending.png`, fullPage: true }); await expect(page.getByText(/Кейс опубликован/)).toBeVisible();
