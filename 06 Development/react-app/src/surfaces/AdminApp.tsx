...
export function AdminApp() {
  ...
  async function loadSession() {
    ...
    try {
      ...
      const allowed = result.actor.allowed_tabs ?? tabs.map((item) => item.id);
      if (!allowed.includes(routeTab())) {
  ...
  async function loadData(filters: AuditFilters = auditFilters) {
    if (!session) return;
    if (session.actor.allowed_tabs && !session.actor.allowed_tabs.includes(tab)) { setData({ items: [] }); return; }
  ...
  const visibleTabs = tabs.filter((item) => !session.actor.allowed_tabs || session.actor.allowed_tabs.includes(item.id));
