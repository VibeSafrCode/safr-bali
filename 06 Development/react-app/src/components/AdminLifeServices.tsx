...
function AdminLifeContent({ userId, csrfToken, locale }: Props) {
  ...
  return <section className="admin-life" aria-labelledby={`${formId}-section-title`}>
    ...
    <div className="admin-life-list">{items.map((item) => <button type="button" key={item.id} disabled={locked} onClick={(event) => openEditor(item, event.currentTarget)}><span><strong>{item.title || shared[item.kind]}</strong><small>{shared[item.kind]} · {t.status[item.publication_status]}</small></span><time dateTime={item.end_date ?? undefined}>{formatLifeDate(item.end_date, locale)}</time></button>)}</div>
    {editorOpen && <form className="admin-life-editor" noValidate aria-busy={pending} onSubmit={(event) => { event.preventDefault(); void write(saved?.publication_status === "PUBLISHED" ? "PUBLISHED" : "DRAFT"); }}>
      ...
      <div className="life-form-grid">
        {field("kind", t.kind, <select {...attrs("kind")} value={draft.kind} onChange={(event) => { const kind = event.target.value as LifeKind; setDraft((current) => ({ ...current, kind, price_unit: kind === "insurance" ? "policy" : "period" })); setErrors({}); }}>{(["housing", "bike", "insurance"] as const).map((kind) => <option key={kind} value={kind}>{shared[kind]}</option>)}</select>)}
        ...
        {field("end_date", lifeDateLabel(draft.kind, "end", locale), <input {...attrs("end_date")} type="date" value={draft.end_date ?? ""} onChange={(event) => update("end_date", event.target.value)} />)}
        {field("price_amount", t.price, <input {...attrs("price_amount")} inputMode="decimal" value={draft.price_amount ?? ""} placeholder="2500000.00" onChange={(event) => update("price_amount", event.target.value)} />)}
        ...
        {field("price_unit", t.unit, <select {...attrs("price_unit")} value={draft.price_unit} onChange={(event) => update("price_unit", event.target.value as LifeDraft["price_unit"])}>{(draft.kind === "insurance" ? ["policy", "period"] as const : ["period", "month", "day"] as const).map((unit) => <option key={unit} value={unit}>{shared[unit]}</option>)}</select>)}
      </div>
