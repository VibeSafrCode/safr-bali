...
function StatusPicker({ label, value, current, options, transitions, locale, onChange, disabled }: { label: string; value: string; current: string; options: string[]; transitions: Record<string, string[]>; locale: "ru" | "en"; onChange: (value: string) => void; disabled: boolean }) {
  ...
  return <div className="crm-status-picker"><span>{label}<button type="button" className="crm-info" aria-expanded={helpOpen} aria-controls={helpId} aria-label={`${label}: ${locale === "ru" ? "справка" : "help"}`} onClick={() => setHelpOpen(!helpOpen)}>i</button></span><details><summary aria-disabled={disabled}><strong>{value}</strong><span aria-hidden="true">⌄</span></summary><div className="crm-status-options" role="listbox" aria-label={label}>{options.map((code) => {
  ...
  })}</div></details><small id={helpId} className="crm-status-help-copy" hidden={!helpOpen}>{currentDescription} {locale === "ru" ? "Код сохраняется без перевода. Справка описывает только рабочее состояние." : "The code is stored unchanged. Help describes workflow only."}</small></div>;
}
