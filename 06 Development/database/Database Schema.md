...
- один `child_user_id` может иметь только одного parent;
- `parent_user_id` не может совпадать с `child_user_id`;
- после первой атрибуции связь неизменяема;
- PostgreSQL trigger запрещает перепривязку и изменение/удаление созданной
  строки связи;
- `users.invited_by_user_id` и строка `referrals` создаются одним use case и
  одной транзакцией;
- browser login не имеет права создавать или изменять связь;
- constraints подготовлены локально, но соответствующая Alembic-миграция не
  применена к production.
...
- баланс рассчитывается backend после блокировки получателя;
- frontend не записывает Points;
- старые операции не изменяются и не пересчитываются;
- PostgreSQL trigger делает ledger append-only и запрещает `UPDATE/DELETE`;
- одинаковый `idempotency_key` не создаёт вторую операцию;
- один заказ не создаёт более одной операции `referral_accrual`;
- snapshot сохраняет правило, по которому рассчитана новая операция;
- старые операции остаются с `NULL` в новых nullable-полях.
