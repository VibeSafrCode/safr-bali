...
### Права чтения для отдельной FX-службы

Проверять импорт конфигурации от **пользователя эффективного unit**, а не
только от root. `safr-bali-pricing-fx.service` работает как `www-data`.
После обновления исходников проверить чтение каждого изменённого Python-модуля
этим пользователем, выполнить штатный refresh и подтвердить следующий
самостоятельный запуск таймера по новой LIVE publication/FX version.
Успешный backend health сам по себе не доказывает работу FX-таймера.

Коррекция 2026-09-15: подтверждённый Git-исходник `app/core/config.py`
оказался `0600 root:root`, поэтому отдельная служба не могла его импортировать.
Founder разрешил **только** service-user read: `0640 root:www-data`, с проверкой,
что группа содержит только `www-data`. Содержимое файла не меняется; `.env`,
секреты и остальные права не затрагиваются. Не применять world-readable
`0644` или рекурсивный chmod как универсальное исправление. Сохранить точные
owner/group/mode при релизе и откате; rollback этой отдельной коррекции —
исходные `0600 root:root`, но он вновь остановит FX и требует осознанного решения.

Справочные доллары с шагом $5 считаются только сервером из того же принятого
ask, что и точный USDT: `ROUND_HALF_UP((IDR / ask) / 5) × 5`. Это приблизительный
эквивалент, не fiat USD quote и не сумма расчёта заказа. При истечении срока
проекции скрываются оба производных значения, IDR остаётся. Схема БД и
исторические коммерческие snapshots для этого изменения не мигрируются.
