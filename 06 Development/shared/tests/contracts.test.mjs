...
test("Astro contract is 50 documents plus one discovery redirect surface", () => {
  assert.equal(report.astroRoutes, 50);
  assert.equal(report.astroDiscoverySurfaces, 51);
});
...
test("ecosystem contract covers 53 routes", () => {
  assert.equal(report.ecosystemRoutes, 53);
});
