...
test("unchanged reviewed hub retains source styles and citations under production CSP", async ({ page }) => {
...
test("unchanged reviewed hub has reachable no-JS source and price contact", async ({ browser, baseURL }) => {
...
test("all bot visa texts and full price tiers render on mobile under strict CSP in both languages and themes", async ({ page }, testInfo) => {
  ...
  const projection = {
    ...
    derived_expires_at: "2099-01-01T00:00:00Z", items: [
      { entity_type: "VISA", entity_key: "E33G", show_price: true, amount_idr: "14000000", display_usdt: "850.01", sku: "express", sort_order: 2, label: { ru: "Экспресс", en: "Express" } },
      { entity_type: "VISA", entity_key: "E33G", show_price: true, amount_idr: "12000000", display_usdt: "720.02", sku: "standard", sort_order: 1, label: { ru: "Стандарт", en: "Standard" } },
    ],
  };
  await page.route("**/*", async (route) => {
  ...
  for (const locale of ["ru", "en"] as const) for (const slug of ["e33g", "d12", "d1-d2", "c1", "voa", "other-visa"]) {
    ...
    if (slug !== "other-visa") {
      ...
      if (slug === "e33g") {
        ...
        await expect(price).toContainText("Rp 12.000.000 (≈ 720.02 USDT)");
        await expect(price).toContainText("Rp 14.000.000 (≈ 850.01 USDT)");
